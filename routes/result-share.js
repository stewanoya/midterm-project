...
const resultShare = (db) => {
  router.get("/:resultID/:shortURL", (req, res) => {
    ...
    const queryString = `SELECT results.id, name, score, title, cover_image_url, category, finish_at as date_completed, quizzes.short_url, COUNT(questions_answers.id) as count
    FROM users
